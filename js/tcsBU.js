// GLOBAL: keep track of selected nodes
var selectedNodes = new Set();

// Click handler for multi‑selection (attached to the circle element)
function onNodeClick(d) {
  // Stop the event from propagating so it doesn't trigger panning
  d3.event.stopPropagation();

  // If shift key is pressed, toggle selection for this node.
  if (d3.event.shiftKey) {
    ...
    // If shift is not pressed, clear any previous selection and select this node.
...
// Helper function to update positions of links and nodes
function manualTick() {
  // Update links so that they follow nodes
  d3.selectAll(".link")
...
// Custom drag behavior that moves all selected nodes together
var customDrag = d3.behavior.drag()
  .on("dragstart", function(d) {
    var evt = d3.event.sourceEvent;
    evt.stopPropagation();
    evt.preventDefault();
    force.alphaTarget(0.3).start();
  })
  .on("drag", function(d) {
    // Determine which nodes to move: if this node is in the selection, move them all; otherwise, move only d.
    var nodesToMove = selectedNodes.has(d) ? Array.from(selectedNodes) : [d];
    nodesToMove.forEach(function(nd) {
      nd.x += d3.event.dx;
      nd.y += d3.event.dy;
      nd.fx = nd.x;
      nd.fy = nd.y;
    });
    manualTick(); // Immediately update link positions.
  })
  .on("dragend", function(d) {
    ...
    // Optionally, after a delay, let the simulation cool down:
    setTimeout(function() {
      force.alphaTarget(0).stop();
    }, 2000);
  });
...
function updateSVG() {
  ...
  var nodeEnter = node.enter().append('g')
    .attr('class', 'node')
    .attr('id', function(d) { return d.name; })
    .call(customDrag); // Attach our custom drag behavior
  
  // Append a circle to each node and attach our onNodeClick handler for multi‑selection
  nodeEnter.append('circle')
    .attr('r', function(d){ return d.radius; })
    .on("click", onNodeClick) // Attach the multi‑selection click handler
  ...
  // Continue with your code for paths, etc.
  ...
  path.enter()
      ...
      .style('fill', function(d) { 
         return d.data.pattern === 'none' ? d.data.color : d.data.pattern;
      });
  if (outlinenodes) {
  ...
  // Instead of calling force.start() here, simply bind the nodes/links:
...
function svgStart() {
  ...
  force = d3.layout.force()
            .charge(function(d){ return d ? d.radius * defaultCharge : defaultCharge; })
            .linkDistance(function(d){ return d ? d.ldist * defaultLinkDistance : defaultLinkDistance; })
            ...
            .size([w, h])
            .on('end', function() { 
                // You might update UI here if needed.
  ...
  */
  ...
  .on("dragstart", function(d) {
      ...
      d.fx = d.x;
      d.fy = d.y;
      force.stop();  // Ensure simulation stays off during drag
  })
  .on("drag", function(d) {
      d.fx = d3.event.x;
      d.fy = d3.event.y;
      force.stop();  // Keep it stopped as you drag
  })
  .on("dragend", function(d) {
      d.fx = d3.event.x;
      d.fy = d3.event.y;
      force.stop();  // Ensure the simulation remains off after drag
  });


// Build network elements and start the simulation:
updateSVG();
force.start();

// After a delay (adjust as needed), stop the simulation and rebind custom drag:
setTimeout(function() {
    force.alphaTarget(0).stop();
    // Detach previous drag events and bind the custom drag
    svg.selectAll(".node").on(".drag", null).call(customDrag);
}, 2000);
